...
SCREENS.purchaseorders = async (page, headless = false) => {
  ...
  page.innerHTML = `
    ...
    <div class="panel" id="pt_ord" hidden>
      <div class="subtabs">
        <button data-p="ord" class="on">Purchase orders</button>
  ...
  // paid — the balance is nothing; paid w/ bal — some has come off but not
  // all; unpaid — nothing has, including a bill just raised.
  ...
  const billPaymentForm = async (bill, done) => {
    dialog(`
      ...
      <h3 class="mt">Supplier billing</h3>
      ...
      <div class="filegrid mt" id="bp_billgrid"><div class="dim">Loading…</div></div>
      <div class="row mt">
        <div style="flex:2"><label>Add a photo or scan</label>
          <input id="bp_billfile" type="file" accept="image/*"></div>
      </div>
    ...
    const paintBillFiles = async () => {
      ...
      if (!grid) return;
      const files = await GET(`/api/purchase-order-bills/${current.id}/files`).catch(() => []);
      grid.innerHTML = files.length ? files.map((f) => `<figure class="filecard">
          <img class="filethumb" src="/api/purchase-order-bill-files/${f.id}" alt=""
            ...
            data-zoom-cap="${esc(current.po_no)}">
          <figcaption><span class="dim">${esc(f.uploaded_by || '')} · ${onDay(f.uploaded_at)}</span>
            <button class="linkbtn del-billfile" data-file="${f.id}">remove</button></figcaption>
        </figure>`).join('') : '<div class="dim">None yet.</div>';
      $$('.del-billfile', grid).forEach((btn) => btn.addEventListener('click', async () => {
        if (!await askFirst('Remove this file from the record?')) return;
        try {
          await DELETE(`/api/purchase-order-bill-files/${btn.dataset.file}`);
          notice('Removed', 'good');
          await paintBillFiles();
        } catch (e) { whoops(e); }
      }));
    };
    await paintBillFiles();

    $('#bp_billfile').addEventListener('change', async (ev) => {
      const file = ev.target.files[0];
      if (!file) return;
      try {
        const dataUrl = await shrink(file, 1600);
        await POST(`/api/purchase-order-bills/${current.id}/files`, { dataUrl });
        notice('Saved 🌸', 'good');
        ev.target.value = '';
        await paintBillFiles();
      } catch (e) { whoops(e); }
...
// Pending customer order's own Open dialog. The shared openOrder below keeps
// its numbers panel and its pick-in-this-order sheet for everybody else that
// opens an order; this tab asked to see a list of orders instead, card by
// card, the way Chat order builds its own basket — so it gets its own copy
// rather than a branch through the shared one.
async function openPendingOrder(id, reload, page) {
...
const resellerList = (part, tier) => async (page) => {
  ...
  // No in-between box: New reseller makes a fresh account on the tab's tier and
  // opens it straight on its own detail screen — the very page an existing
  // account opens to. Everything is filled there, in one place: the real name,
  // the numbers, the tier, the photo, the ID and papers. The starter name is
