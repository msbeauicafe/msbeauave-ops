...
// HR and the operations manager: the people half of the company, and only
// that. The team list, HR, the cutoff, and their own record.
//
// Both of them held an admin sign-in until this existed, which is how two
// people came to hold the pricelist, the company's money and the sign-ins as
// the price of being able to approve somebody's leave. This list is not the
// only thing standing there — the database refuses an 'hr' outside the
// functions those four screens call.
export const PEOPLE = ['admin', 'hr'];
// A cutoff and the two ledgers behind it. The office ran payroll before there
// was an HR role and still can.
export const PAYWORK = ['admin', 'office', 'hr'];
...
export const OWN_RECORD = ['admin', 'employee', 'observer', 'orderdesk', 'datacoord', 'hr'];
