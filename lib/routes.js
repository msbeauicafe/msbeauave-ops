...
on('GET', '/api/products', [...STAFF, 'observer'], async ({ db, res, query }) => {
  ...
  // An array, not an object: order matters and an object would throw it away
  // on the way out.
  //
  // Tier 1, 2, 3 here is the same RD, SUB RD, PD... order Pricelists reads
  // down as columns — the same price code lands in the same spot on both
  // screens, rather than each product closing its prices up in whatever box
  // they happened to be typed into on its own form.
  //
  ...
  // finding it anywhere on the list you typed it from. It reads last, the way
  // Pricelists' own SRP column always does.
  const wanted = query.prices === '1'
    ? `, coalesce((select jsonb_agg(jsonb_build_object('code', x.code, 'price', x.price)
                                    order by x.pos, x.code)
                     from (select pp.code, pp.price, pc.sort as pos
                             from product_prices pp
                             join price_codes pc on pc.code = pp.code
                            where pp.sku = s.sku
                           union all
                           select 'SRP', s.srp, 999999
...
on('GET', '/api/pricelist', ADMIN, async ({ db, res }) => {
  const [codes, rows] = await queryAll(db, [
    ...
    [`select p.sku, p.name, p.brand, p.unit_type, p.unit_cost, p.srp,
