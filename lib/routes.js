...
import { on, ok, fail, send, ANYONE, ADMIN, STOCK, TILL, STAFF, SUPER, RESELLER } from './router.js';
...
// business answering.
on('GET', '/api/branches', STAFF, async ({ db, res, user }) => {
  const r = await db.query(
...
on('POST', '/api/clock/by-pin', STAFF, async ({ db, res, body }) => {
