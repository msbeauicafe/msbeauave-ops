...
on('POST', '/api/receive', STOCK, async ({ db, res, body }) => {
  ...
  const received = await db.query('select receive_stock($1,$2,$3,$4,$5,$6) as id',
...
on('GET', '/api/team/hours', ADMIN, async ({ db, res, query }) => {
  ...
  const r = await db.query('select * from hours_worked($1,$2)', [from, to]);
...
on('POST', '/api/team/bulk', ADMIN, async ({ db, res, body }) => {
  ...
  const r = await db.query('select add_employees($1::jsonb) as result',
