...
on('GET', '/api/resellers/:id', ADMIN, async ({ db, res, params }) => {
  ...
  const [account, invoices, events, documents, credits] = await queryAll(db, [
    ...
    ['select * from reseller_credits where reseller_id = $1 order by at desc limit 50', [id]],
  ]);
...
// One payment against the whole account, applied oldest-invoice-first;
// whatever is left over becomes credit that the next invoice draws on
// automatically.
on('POST', '/api/resellers/:id/payment', ADMIN, async ({ db, res, params, body }) => {
  const r = await db.query('select pay_account($1,$2,$3) as result',
    [Number(params.id), Number(body.amount),
     body.paid_on || new Date().toISOString().slice(0, 10)]);
