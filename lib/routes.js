...
on('GET', '/api/products', [...STAFF, 'observer'], async ({ db, res, query }) => {
  ...
  const wanted = query.prices === '1'
    ? `, coalesce((select jsonb_agg(jsonb_build_object('code', pp.code, 'price', pp.price)
                                    order by pp.position, pp.code)
                     from product_prices pp where pp.sku = s.sku), '[]'::jsonb) as prices`
    : '';
