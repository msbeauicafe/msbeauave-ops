...
// A shop's own picture on their card. Shrunk once here, to 3:4 — the shape
// the card is, so the browser is never asked to crop a wide photograph into a
// tall frame and lose the middle of it. Everything after this is a read, and
// the reason a read stays cheap.
on('POST', '/api/resellers/:id/photo', ADMIN, async ({ db, res, params, body }) => {
  ...
  const bytes = await sharp(uploaded)
    .rotate()
    .resize({ width: 240, height: 320, fit: 'cover', position: 'attention',
