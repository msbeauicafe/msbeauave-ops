...
on('GET', '/api/purchase-order-bills', STOCK, async ({ db, res }) => {
  const r = await db.query(
    `select * from purchase_order_bill_balances
      order by (balance <= 0), coalesce(due_date, invoice_date), id`);
  ok(res, r.rows);
});
...
on('GET', '/api/purchase-order-bills/:id/payments', STOCK, async ({ db, res, params }) => {
  const r = await db.query(
    `select id, amount, paid_on, method, note from purchase_order_bill_payments
      where bill_id = $1 order by paid_on, id`, [Number(params.id)]);
  ok(res, r.rows);
});
