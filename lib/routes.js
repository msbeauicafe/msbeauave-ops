...
import { on, ok, fail, send, ANYONE, ADMIN, STOCK, TILL, STAFF, SUPER, CLOCKDEV,
  RESELLER, PERSON, NOTICEBOARD } from './router.js';
...
on('GET', '/api/products/:sku/batches', STAFF, async ({ db, res, params, query }) => {
  const r = await db.query(
...
on('GET', '/api/branch-stock', STAFF, async ({ db, res, query }) => {
  const r = await db.query(
...
on('GET', '/api/reseller-tiers', ADMIN, async ({ db, res }) => {
...
on('GET', '/api/team', CLOCKDEV, async ({ db, res, user }) => {
...
// business answering.
on('GET', '/api/branches', CLOCKDEV, async ({ db, res, user }) => {
  const r = await db.query(
...
on('POST', '/api/clock/by-pin', CLOCKDEV, async ({ db, res, body }) => {
  ...
  const done = await db.query('select clock_toggle($1) as result', [person.id]);
...
on('POST', '/api/clock', CLOCKDEV, async ({ db, res, body }) => {
  ...
  const done = await db.query('select clock_toggle($1) as result', [id]);
  ok(res, done.rows[0].result);
});
...
// The door has matched somebody. Same shape of answer as the PIN pad, and the
// same silence about why when it says no.
on('POST', '/api/clock/by-finger', CLOCKDEV, async ({ db, res, body, user }) => {
  ...
  try {
    const r = await db.query('select clock_by_finger($1,$2) as result', [id, branch || null]);
    ok(res, r.rows[0].result);
  } catch (e) {
...
on('POST', '/api/team/pins', ADMIN, async ({ db, res, body }) => {
  ...
  const people = await db.query(
    `select id, name, position from employees
      where ended_on is null and id > $1 ${everyone ? '' : 'and pin_hash is null'}
  ...
  const left = await db.query(
    everyone
      ? `select count(*)::int as n from employees where ended_on is null and id > $1`
      : `select count(*)::int as n from employees
          where ended_on is null and pin_hash is null and id > $1`,
    [mark]);
...
on('GET', '/api/team/hours', ADMIN, async ({ db, res, query }) => {
...
on('GET', '/api/team/shifts', ADMIN, async ({ db, res, query }) => {
  const to = query.to || today();
  const from = query.from || to;
  const r = await db.query(
...
on('GET', '/api/team/:id/photo', CLOCKDEV, async ({ db, res, params, query }) => {
  const r = await db.query(
...
// ---------------------------------------------------------------------------
on('GET', '/api/hr', ADMIN, async ({ db, res }) => {
  ...
  ok(res, {
    ...
    figures: {
      ...
      unpaid: here.filter((p) => p.salary == null).length,
...
// ---------------------------------------------------------------------------
on('GET', '/api/my', PERSON, async ({ db, res }) => {
...
on('GET', '/api/my/photo', PERSON, async ({ db, res }) => {
  const r = await db.query(
    `select p.mime, p.bytes, p.updated_at from employee_photos p
      where p.employee_id = current_employee()`);
  const photo = r.rows[0];
  if (!photo) return fail(res, 404, 'No photograph.');
  res.writeHead(200, {
    'Content-Type': photo.mime,
...
// cannot check.
// ---------------------------------------------------------------------------
on('GET', '/api/hr/attendance', ADMIN, async ({ db, res, query }) => {
...
// looked for.
on('GET', '/api/hr/attendance/summary', ADMIN, async ({ db, res, query }) => {
  const to = query.to || today();
