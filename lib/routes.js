...
on('GET', '/api/purchase-orders/:id', STOCK, async ({ db, res, params }) => {
  const [po, lines, receipts] = await queryAll(db, [
    ...
    // Every batch this order's quick-received cell has ever created — the box
    // number and expiry are made up on the spot rather than asked for, so
    // this is the only place they are still on the screen to check. Lackings
    // is read the same way the Lackings cell reads it, just at the moment
    // each receipt landed rather than now: the order's qty for that sku,
    // less everything received on it up to and including this one.
    [`select b.batch_no, b.qty_received, b.expiry, b.received_at, p.name,
             greatest(l.qty - sum(b.qty_received) over (
               partition by b.sku order by b.received_at
               rows between unbounded preceding and current row), 0) as lackings_after
        from batches b
