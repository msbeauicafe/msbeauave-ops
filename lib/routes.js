...
on('GET', '/api/purchase-orders', STOCK, async ({ db, res, query }) => {
  const r = await db.query(
    `select o.*, s.name as supplier, s.brand_name, s.tin, s.address, s.contact,
            ...
            (select count(*)::int from purchase_order_bills b
              where b.po_id = o.id and b.paid_on is not null) as bills_paid
       from purchase_orders o
...
on('GET', '/api/purchase-order-bills', STOCK, async ({ db, res }) => {
  const r = await db.query(
    `select b.*, o.po_no, s.name as supplier, s.brand_name
       ...
       join suppliers s on s.id = o.supplier_id
      order by (b.paid_on is not null), coalesce(b.due_date, b.invoice_date), b.id`);
  ok(res, r.rows);
});
...
on('POST', '/api/purchase-order-bills/:id/paid', STOCK, async ({ db, res, params, body }) => {
  await db.query('select set_po_bill_paid($1,$2)', [Number(params.id), body.paid !== false]);
  ok(res, { saved: true });
});
