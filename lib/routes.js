...
on('GET', '/api/books/accounts', ADMIN, async ({ db, res }) => {
  const r = await db.query(
    'select code, title, type, normal_side, active from coa_accounts order by type, code, title');
  ok(res, r.rows);
});
...
on('POST', '/api/books/bills/:id/pay', ADMIN, async ({ db, res, params, body }) => {
  try {
    const r = await db.query('select pay_bill($1,$2,$3,$4,$5) as id',
...
on('POST', '/api/books/expenses', ADMIN, async ({ db, res, body }) => {
  ...
  try {
    const r = await db.query('select record_expense($1,$2,$3,$4::jsonb) as id',
