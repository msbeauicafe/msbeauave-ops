...
on('GET', '/api/products', [...STAFF, 'observer'], async ({ db, res, query }) => {
  ...
  const wanted = query.prices === '1'
    ? `, coalesce((select jsonb_object_agg(pp.code, pp.price)
...
// Several of a product's prices at once, because they are typed at once. Each
// is set on its own; one bad figure does not throw away the four beside it.
on('PUT', '/api/products/:sku/prices', ['admin', 'office'], async ({ db, res, params, body }) => {
  const rows = (body.prices || []).filter((p) => p && p.code);
  for (const row of rows) {
    await db.query('select set_price($1,$2,$3)',
      [params.sku, row.code, Number(row.price) || 0]);
  }
