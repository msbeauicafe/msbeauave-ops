...
on('GET', '/api/products/:sku/batches', STAFF, async ({ db, res, params, query }) => {
  const r = await db.query(
    `select * from batch_detail
      where sku = $1 and ($2::bigint is null or branch_id = $2)
...
on('POST', '/api/receive', STOCK, async ({ db, res, body }) => {
  ...
  const split = await db.query(
    `select pool, on_hand from stock where batch_id = $1
      and branch_id = coalesce($2, default_branch()) order by pool`,
...
on('GET', '/api/takings-by-branch', ADMIN, async ({ db, res, query }) => {
  const r = await db.query(
    `select * from takings_by_branch
      where business_date between $1 and $2 limit 400`,
    ...
    `select c.sku, c.name, c.brand, c.category, c.retail_price,
            ...
            coalesce((select sum(s.on_hand - s.committed)
                       ...
                       where b.sku = c.sku and s.pool = 'shop'
                         ...
                         and s.branch_id = coalesce($2, default_branch())), 0)::int
              as on_shelf
       from shop_catalog c
...
// ---------------------------------------------------------------------------
on('GET', '/api/branches', STAFF, async ({ db, res }) => {
  ok(res, (await db.query('select * from branch_list')).rows);
});
...
// ---------------------------------------------------------------------------
on('POST', '/api/clock', STAFF, async ({ db, res, body }) => {
...
on('POST', '/api/team/pins', ADMIN, async ({ db, res, body }) => {
  ...
  const people = await db.query(
    `select id, name, position from employees
      where ended_on is null ${everyone ? '' : 'and pin_hash is null'}
  ...
  const left = await db.query(
    `select count(*)::int as n from employees
      where ended_on is null and pin_hash is null`);
  ok(res, { issued, remaining: everyone ? 0 : left.rows[0].n });
});
