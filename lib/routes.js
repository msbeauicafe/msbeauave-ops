...
on('POST', '/api/shop/join', ANYONE, async ({ req, res, body }) => {
  ...
  const customer = { id: r.rows[0].id, name };
  res.setHeader('Set-Cookie', shopperCookie(customer, req));
  ok(res, { customer: { name, points: 0, tier: 'Bronze' } });
});
...
on('GET', '/api/shop/me', ANYONE, async ({ req, res }) => {
  ...
  ok(res, {
    ...
    // Nothing can be in any of these until ordering exists. They are sent as
    // zeroes rather than omitted so the page never has to guess.
    purchases: { toPay: 0, toCollect: 0, toReceive: 0, toRate: 0 },
    vouchers: 0,
  });
});
